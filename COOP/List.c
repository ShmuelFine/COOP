...
DEF_CTOR(GenericList, MEM_SIZE_T dataTypeSize, List_ElementType enumTag)
{
	...
	CREATE(ListIter, begin), _this CALL;
	CREATE(ListIter, end),_this CALL;
...
MEM_FUN_IMPL(GenericList, print)
{
	...
	bool at_end = false;
	MFUN(it, equals), (Iterator*)it_end, & at_end CALL;
...
DEF_DERIVED_CTOR(ListIter, Iterator,void *container_ptr) SUPER, ITER_BIDIRECTIONAL, container_ptr ME
{
	_this->list = NULL;
	_this->node = NULL;
}
