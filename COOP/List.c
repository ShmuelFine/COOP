...
{
    MFUN(_this, clear) CALL;
}
END_DTOR

/* ======================== Helpers ======================== */
// TODO: CHECK IF ELEMENT TYPE IS OBJSPTR. IF YES, CALL INCREF HERE.
...
/* ---- pop_back ---- */
// TODO: VERIFY THIS BEHAVIOR IS CONSISTENT WITH objSPtr DESIGN.
...
DEF_DERIVED_CTOR(ListIter, Iterator) SUPER,(ITER_BIDIRECTIONAL) ME
{
    _this->list = NULL;
    _this->node = NULL;
}
...
FUN_OVERRIDE_IMPL(ListIter, Iterator, distance, Iterator* other, ptrdiff_t* out_dist)
{
...
IMPL_SPECIFIC_LIST_TYPE_xTORs(char);
IMPL_SPECIFIC_LIST_TYPE_FUNCTIONS(char);

IMPL_SPECIFIC_LIST_TYPE_xTORs(float);
IMPL_SPECIFIC_LIST_TYPE_FUNCTIONS(float);

IMPL_SPECIFIC_LIST_TYPE_xTORs(objSPtr);
