...
/* ===== Linear algebraic matrix-multiply =====
   A: _this  (H_THIS ª W_THIS) | B: other  (W_THIS ª W_OTHER) | Out: out  (H_THIS ª W_OTHER).
...
MEM_FUN_IMPL(GrayImage, mul_mat, GrayImage const* other, GrayImage* out)
{
    ...
    /* Dimension check: inner dims must match; output must be H_THIS ª W_OTHER */
    THROW_MSG_UNLESS(_this->width == other->height, "Inner dims mismatch: A.width must equal B.height");
    THROW_MSG_UNLESS(out->height == _this->height && out->width == other->width, "Output size must be (A.height ª B.width)");
